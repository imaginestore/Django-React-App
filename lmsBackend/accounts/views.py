...
class UserRegistrationView(APIView):
  ...
  def post(self, request, format=None):
    ...
    return Response({'token':token, 'user_id':user.id, 'user_type':user.usertype, 'msg':'Registration Successful'}, status=status.HTTP_201_CREATED)
...
class UserLoginView(APIView):
  ...
  def post(self, request, format=None):
    ...
    if user is not None:
      token = get_tokens_for_user(user)
      return Response({'token':token, 'user_id':user.id, 'user_type':user.usertype, 'user_name':user.username, 'msg':'Login Success'}, status=status.HTTP_200_OK)
    else:
